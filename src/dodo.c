...
int days_diff(time_t a, time_t b) {
...
char* DDLGetMakeStash() {
  static char dodo_stash[] = "stash/";
  char *stashd;
  DIR* dir;
  if(!DODO_ROOT) {
    if(!DDLGetMakeRoot()) return NULL;
  }
...
DDList* FileToDDL(char* name) {
  ...
  struct stat sa = {0};
  time_init();
  if(!DDLGetMakeStash()) return NULL;
...
void printn(DDNode* dn, int depth) {
  ...
  static char heart[]   = "♡";
  static char star[]    = "★";
  static char sp_todo[] = "\33[35m□\33[39m";
  static char sp_done[] = "\33[32m✓\33[39m";
  static char sp_hipr[] = "\33[36m!\33[39m";
  static char sp_star[] = "\33[33m★\33[39m";
  static char sp_love[] = "\33[31m♡\33[39m";
  static char sp_note[] = "\33[34m•\33[39m";
...
int main(int argc, char *argv[]) {
  if(argc < 2) return -1;
  DDList* ddl = FileToDDL(argv[1]);
  printd(ddl);
  DDLToFile(ddl, argv[1]);
  return 0;
}
